...
Quad trees are often used to partition a two-dimensional space by recursively subdividing
...
Your task is to implement a function that will take two quadtrees and return a quadtree that represents
the logical OR (or union) of the two trees.
...
class Solution:
    def intersect(self, quadTree1, quadTree2):
        ...
        if all([a.isLeaf for a in [top_left, top_right, bottom_left, bottom_right]]):
            if all([a.val for a in [top_left, top_right, bottom_left, bottom_right]]) or all(
                    [not a.val for a in
